...
class PulsingCircleRenderer: Renderer {
	...
	var vertexFunctionName: String { return "pulsing_circle_vertex_main" }
	var fragmentFunctionName: String { return "pulsing_circle_fragment_main" }
	...
	func add(timestep: TimeInterval) {
		for i in 0..<self.shapes.count {
			self.shapes[i].add(timestep: timestep)
		}
		self.updateUniformBuffer()
	}
}
